...
instance Monad RWSP where
  ...
  m >>= f = RWSP (\r s -> let (a, w, s') = runRWSP m r s in runRWSP (f a) r s')
...
withP r' m = RWSP (\r s -> let (a, w, s') = runRWSP m r s in runRWSP m r' s')

-- adds some write data to accumulator
tellP :: WriteData -> RWSP ()
tellP w = RWSP (\r s ->let (a, w', s') = runRWSP (return w) r s in ((), w++w', s'))
...
sampleP2 :: RWSP Answer
sampleP2 = 
  do  putP 1.0
      s1 <- getP
      return $ "s = " ++ show s1

sampleP3 :: RWSP Answer
sampleP3 = 
  do  tellP "Hej"
      r2 <- withP 5 askP
      r3 <- askP
...
withE r' m = RWSE (\r s -> case runRWSE m r s of
  Right (a, w, s') -> runRWSE m r' s'
  Left e -> Left e)

tellE :: WriteData -> RWSE ()
tellE w = RWSE (\r s -> case runRWSE (return w) r s of
  Right (a, w', s') -> Right ((), w++w', s')
  Left e -> Left e)
