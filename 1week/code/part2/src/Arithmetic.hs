...
extendEnv var x env = \_v -> if _v == var then Just x else env _v
...
evalFull (Pow exp1 exp2) env = case evalFull exp1 env of
  x -> case evalFull exp2 env of
    y -> if y >= 0 then if x /= 0 then x^y
                                  else 0^y
                   else error "Cannot set minus power"

evalFull (If test yes no) env = if not (evalFull test env == 0) then evalFull yes env
...
evalErr (If test yes no) env = case evalErr test env of
  Right val1 -> if not (val1 == 0) then case evalErr yes env of
